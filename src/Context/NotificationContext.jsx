...
const SEED_NOTIFICATIONS = [
  {
    ...
    actorAvatar: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?auto=format&fit=crop&w=120&q=80",
  ...
  },
  {
    ...
    actorAvatar: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&w=120&q=80",
    actorUsername: "@jamief",
    content: 'replied to your comment: "I used coconut cream and it worked great!"',
  ...
  },
  {
    ...
    actorAvatar: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&w=120&q=80",
  ...
  },
  {
    ...
    type: "repost",
    actorName: "Nico Tan",
    actorAvatar: "https://images.unsplash.com/photo-1504593811423-6dd665756598?auto=format&fit=crop&w=120&q=80",
...
const normalizeNotification = (notification) => {
  ...
  return {
    ...
    actorAvatar: notification.actorAvatar || notification.avatar || DEFAULT_AVATAR,
    actorUsername: notification.actorUsername || "",
    content: notification.content || notification.caption || "interacted with your post.",
    caption: notification.caption || notification.content || "There is new activity on your account.",
    timestamp,
...
export function NotificationProvider({ children }) {
  ...
  useEffect(() => {
    ...
    setPopupQueue((prev) => {
      ...
      const additions = unreadVisible.filter((item) => !existingIds.has(item.id));
  ...
  const createNotification = useCallback((notificationInput) => {
    ...
    setPopupQueue((prev) => [entry, ...prev.filter((item) => item.id !== entry.id)].slice(0, 4));
  ...
  const hideNotification = useCallback((id) => {
    setNotifications((prev) =>
      prev.map((notification) =>
        notification.id === id ? { ...notification, hidden: true, read: true } : notification
      )
    );
    setPopupQueue((prev) => prev.filter((notification) => notification.id !== id));
  }, []);

  const unhideNotification = useCallback((id) => {
    setNotifications((prev) =>
      prev.map((notification) =>
        notification.id === id ? { ...notification, hidden: false } : notification
      )
    );
  }, []);
    ...
    const scenarios = [
      {
        ...
        actorAvatar: "https://images.unsplash.com/photo-1488426862026-3ee34a7d66df?auto=format&fit=crop&w=120&q=80",
        actorUsername: "@ellareyes",
        content: 'liked your recipe "Spicy Tuna Pasta".',
        caption: "Your recipe just got another heart.",
        targetRoute: "/feed",
        images: ["https://images.unsplash.com/photo-1559847844-5315695dadae?auto=format&fit=crop&w=600&q=80"],
      },
      {
        type: "comment",
        actorName: "Paolo Santos",
        actorAvatar: "https://images.unsplash.com/photo-1502685104226-ee32379fefbe?auto=format&fit=crop&w=120&q=80",
      ...
      },
      {
        ...
        actorAvatar: "https://images.unsplash.com/photo-1544723795-3fb6469f5b39?auto=format&fit=crop&w=120&q=80",
