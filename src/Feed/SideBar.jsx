...
import {
  Home, Bell, Heart, Archive, History, Search,
  ChevronLeft, LogOut, Plus, Sparkles,
  X, MessageSquare, ChevronRight, Menu,
} from "lucide-react";
...
const MORE_NAV = [
  ...
  { icon: Sparkles,label: "Gastronome AI", path: "/chatbot", orange: true },
];
...
function DesktopSidebar({ onNewPost, hasNotifications }) {
  ...
  const blueGradientText   = "bg-gradient-to-r from-[#0060A9] to-[#00B4FA] bg-clip-text text-transparent";
  const orangeGradientText = "bg-gradient-to-r from-[#F57600] to-[#F0AE35] bg-clip-text text-transparent";
  ...
  return (
    <>
      <CreatePostModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onPost={onNewPost} />
      <LogOutModal
        isOpen={showLogoutConfirm}
        onConfirm={() => { setShowLogoutConfirm(false); navigate("/home"); }}
      ...
      >
        ...
        >
          <ChevronLeft size={20} style={{ transform: chevronFlipped ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.3s ease" }} />
        </button>
        ...
        <div className="flex flex-col h-full w-full overflow-hidden">
          ...
          <div className="p-4 flex flex-col items-center z-20">
            <div className="flex items-center mb-6 h-12 w-full">
              <div className="w-[48px] h-12 flex items-center justify-center flex-shrink-0">
                <img src={LogoImage} alt="Gastro Logo" className="w-10 h-10 object-contain" />
              </div>
              <div className={`${textTransitionClass} flex-1 relative h-12`}>
                <AnimatePresence mode="wait">
                  {isHome ? (
                    <motion.div key="logo-text"
                      initial={shouldAnimateHeader ? { opacity: 0, x: -15 } : false}
                    ...
                    >
                      <div className="w-44"><SearchBar /></div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
          ...
          <nav className="flex-1 relative pt-2">
              <DesktopNavItem
                icon={<img src={AILogo} alt="AI" className="w-6 h-6 object-contain" />}
                label={<span>Gastronome AI <Sparkles size={11} className="inline ml-1.5 text-[#F57600]" /></span>}
              ...
              {!collapsed && (
                <>
                  <div className="flex items-center w-full h-10">
                    ...
                    <span className="text-[11px] font-black uppercase tracking-widest text-gray-400 whitespace-nowrap">Chat History</span>
                  </div>
          ...
          <div className="mt-auto p-3">
            <div className={`w-full rounded-2xl flex items-center h-14 overflow-hidden transition-all duration-300 shadow-lg relative
              ${isProfile ? "bg-white border-2 border-[#0060A9] text-[#0060A9]" : "bg-gradient-to-r from-[#0060A9] to-[#00B4FA] text-white"}`}>
              {isProfile && (
                <motion.div animate={{ opacity: [0.05, 0.15, 0.05] }} transition={{ repeat: Infinity, duration: 2 }}
                  className="absolute inset-0 bg-[#0060A9] pointer-events-none" />
              )}
              <button onClick={() => navigate("/profile")} className="flex items-center flex-1 h-full min-w-0 overflow-hidden active:scale-[0.98] transition-transform">
                <div className="w-[54px] flex-shrink-0 flex items-center justify-center z-10">
                  {userAvatar ? (
                    <img src={userAvatar} alt="Profile" className={`w-9 h-9 rounded-full object-cover block ${isProfile ? "border-2 border-[#0060A9]" : "border-2 border-white/40"}`} />
                  ) : (
                ...
                <div className={`${textTransitionClass} flex flex-col items-start min-w-0 z-10`}>
                  <span className={`font-bold text-sm truncate w-full ${isProfile ? "text-[#0060A9]" : "text-white"}`}>{userName || "User"}</span>
                  {isProfile && <span className="text-[10px] font-medium uppercase tracking-widest opacity-70">Active Profile</span>}
                </div>
              </button>
              <button
              ...
              >
                <LogOut size={17} className="transition-transform duration-200 group-hover/logout:translate-x-0.5" />
              </button>
            </div>
          </div>
        </div>
...
function DesktopNavItem({ icon, label, isActive, collapsed, onClick, blueGradientText }) {
  const textClass = `transition-all duration-400 overflow-hidden whitespace-nowrap ${
    collapsed ? "max-w-0 opacity-0 ml-0" : "max-w-[200px] opacity-100 ml-3"
  }`;
  return (
    <div onClick={onClick} className="group relative flex items-center h-14 cursor-pointer transition-colors">
      ...
      />
      <div className={`w-16 h-14 flex items-center justify-center flex-shrink-0 z-10 transition-colors duration-300 ${isActive ? "text-[#0060A9]" : "text-gray-400 group-hover:text-[#0060A9]"}`}>
        {icon}
      </div>
      <span className={`${textClass} text-[15px] z-10 transition-colors duration-200 ${isActive ? `font-bold ${blueGradientText}` : "text-gray-500 font-medium group-hover:text-[#0060A9]"}`}>
        {label}
      </span>
      {collapsed && (
...
function MobileBottomNav({ onNewPost, hasNotifications, onMobileSearch }) {
  ...
  useEffect(() => {
    ...
    const onTouchEnd = (e) => {
      ...
      const dy = Math.abs(e.changedTouches[0].clientY - screenSwipeStartY.current);
  ...
  return (
    <>
        ...
        {searchOpen && (
          <>
            ...
            <motion.div
              ...
              style={{ height: "calc(100dvh - 84px - env(safe-area-inset-bottom,0px) - 32px)" }}
            >
              {/* Input row */}
              <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 flex items-center gap-3 px-4 py-3 flex-shrink-0">
                ...
                {searchQuery ? (
                  <button
                    onClick={() => { setSearchQuery(""); onMobileSearch?.(""); }}
                  ...
                  <button
                    onClick={() => { setSearchOpen(false); setSearchQuery(""); onMobileSearch?.(""); }}
                    className="text-gray-400 hover:text-red-400 transition-colors"
                  >
                    <X size={16} />
                  </button>
          ...
          <>
            ...
            <motion.div
              ...
              style={{
                width: "80vw",
                maxWidth: 320,
                boxShadow: "8px 0 40px -4px rgba(0,96,169,0.18), 2px 0 0 rgba(0,96,169,0.06)",
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100 flex-shrink-0"
                style={{ background: "linear-gradient(135deg, rgba(245,118,0,0.06) 0%, rgba(240,174,53,0.06) 100%)" }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-black text-gray-800 leading-none">Gastronome AI</p>
                    <p className="text-[10px] text-gray-400 font-medium mt-0.5">Your recipe assistant</p>
                  </div>
                </div>
              ...
              <div className="px-4 pt-4 pb-3 flex-shrink-0">
                <button
                  onClick={() => { navigate("/profile"); setPanelOpen(false); }}
                ...
                >
                  ...
                  <div className="flex-shrink-0">
                    {userAvatar ? (
                      <img src={userAvatar} alt="Profile" className="w-9 h-9 rounded-full object-cover border-2 border-white/40" />
                    ) : (
              ...
              <div className="px-3 pt-3 pb-2 flex-shrink-0">
                <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 px-2 mb-2">Pages</p>
                {EXTRA_NAV.map((item) => {
                  const isActive = location.pathname === item.path;
                  return (
                    <button key={item.path}
                      onClick={() => { navigate(item.path); setPanelOpen(false); }}
                      ...
                      <span className={`text-sm font-bold ${isActive ? "text-[#0060A9]" : "text-gray-600"}`}>{item.label}</span>
                      {isActive && <ChevronRight size={14} className="ml-auto text-[#0060A9]/40" />}
                    </button>
                  );
                })}
              </div>
              ...
              <div className="flex flex-col flex-1 min-h-0 px-3 pt-3">
                <div className="flex items-center justify-between px-2 mb-2 flex-shrink-0">
                  <p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Chat History</p>
                </div>

                {/* Scrollable list */}
                <div
                  className="flex-1 overflow-y-auto pr-1"
                  style={{ scrollbarWidth: "thin", scrollbarColor: "#F57600 #FFF3E6" }}
                >
                  <style>{`
                  ...
                  <div className="chat-scroll h-full overflow-y-auto space-y-0.5 pb-2">
                    {HISTORY_ITEMS.map((item, i) => (
                      <button key={i}
                        onClick={() => { navigate("/chatbot"); setPanelOpen(false); }}
                      ...
                      >
                        ...
                        <span className="text-xs text-gray-500 font-medium flex-1 line-clamp-2 leading-snug">{item}</span>
                        <ChevronRight size={12} className="text-gray-300 group-hover:text-[#0060A9] flex-shrink-0 transition-colors" />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="px-4 py-4 border-t border-gray-100 flex-shrink-0">
                <motion.button
                  whileTap={{ scale: 0.96 }}
                  onClick={() => { setPanelOpen(false); setShowLogoutConfirm(true); }}
      ...
      <div className={`fixed z-[130] lg:hidden select-none ${location.pathname === "/chatbot" ? "hidden" : ""}`}
        style={{ bottom: "calc(84px + env(safe-area-inset-bottom, 0px) + 12px)", right: "16px" }}>
        ...
        <motion.button
          ...
          style={{
            ...
            boxShadow: "0 4px 20px rgba(245,118,0,0.45), 0 2px 8px rgba(0,0,0,0.12)",
          }}
        >
          {/* Pulse ring */}
          <motion.span
          ...
          />
          <img src={AILogo} alt="AI" className="w-7 h-7 object-contain relative z-10" />
        </motion.button>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[120] lg:hidden">
        ...
        >
          <div className="flex items-center justify-around px-4 pt-3 pb-2">
            ...
            <XNavBtn icon={Home} isActive={!searchOpen && location.pathname === "/feed"} onClick={() => navTo("/feed")} label="Home" />

            {/* Search — only this one highlights while overlay is open */}
            <XNavBtn icon={Search} isActive={searchOpen} onClick={() => {
              if (searchOpen) { setSearchQuery(""); onMobileSearch?.(""); }
              setSearchOpen(s => !s);
            }} label="Search" />
            ...
            <div className="flex flex-col items-center">
              <motion.button
                whileTap={{ scale: 0.88 }}
                onClick={() => { setSearchOpen(false); setIsModalOpen(true); }}
            ...
            <XNavBtn icon={Bell} isActive={!searchOpen && location.pathname === "/notifications"} onClick={() => navTo("/notifications")} label="Notification" badge={hasNotifications} />

            {/* Menu — opens the slide panel; inactive while search is open */}
            <button
              onClick={() => { setSearchOpen(false); setPanelOpen(true); }}
            ...
            >
              <div className="relative">
                <Menu
                  ...
                  className={!searchOpen && panelOpen ? "text-[#0060A9]" : "text-gray-500"}
                />
              </div>
              <span className={`text-[9px] font-bold ${!searchOpen && panelOpen ? "text-[#0060A9]" : "text-gray-400"}`}>Menu</span>
            </button>
...
function XNavBtn({ icon: Icon, isActive, onClick, label, badge }) {
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-0.5 py-1 px-2 rounded-xl transition-all active:scale-90">
      <div className="relative">
        <Icon
          size={24}
      ...
      <span className={`text-[9px] font-bold ${isActive ? "text-[#0060A9]" : "text-gray-400"}`}>{label}</span>
    </button>
  );
}
...
// ─────────────────────────────────────────────────────────────────────────────
export default function Sidebar({ onNewPost, hasNotifications = false, onMobileSearch }) {
  return (
    <>
      <DesktopSidebar onNewPost={onNewPost} hasNotifications={hasNotifications} />
      <MobileBottomNav onNewPost={onNewPost} hasNotifications={hasNotifications} onMobileSearch={onMobileSearch} />
    </>
  );
}