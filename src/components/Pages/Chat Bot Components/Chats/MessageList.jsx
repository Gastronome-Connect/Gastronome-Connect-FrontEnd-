...
function ActionBar({ time }) {
  return (
    <div className="flex items-center gap-1 sm:gap-1.5 pl-9 sm:pl-20">
      {[
        <svg key="copy" className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          <rect x="9" y="9" width="13" height="13" rx="2" />
          <path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1" />
        </svg>,
        <svg key="up" className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M14 9V5a3 3 0 00-3-3l-4 9v11h11.28a2 2 0 002-1.7l1.38-9a2 2 0 00-2-2.3H14z" />
          <path strokeLinecap="round" strokeLinejoin="round" d="M7 22H4a2 2 0 01-2-2v-7a2 2 0 012-2h3" />
        </svg>,
        <svg key="down" className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        ...
        <svg key="regen" className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
        </svg>,
      ].map((icon, i) => (
        <button key={i} className="p-1 text-gray-400 hover:text-gray-600 transition-colors">
          {icon}
        </button>
      ))}
  ...
  return (
    <div className="flex-1 overflow-y-auto flex flex-col py-3 sm:py-4">
      {messages.map((msg) => {
        if (msg.role === "user") {
          return (
            <UserMessageBubble key={msg.id} message={msg.text} time={msg.time} />
          );
        }
