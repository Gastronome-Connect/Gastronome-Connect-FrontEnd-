...
import { Bell, Heart, MessageCircle, Repeat2, UserPlus, X } from "lucide-react";
...
function FeedNotificationPopup({ notification, onDismiss }) {
  ...
  const iconClass = TYPE_COLOR_MAP[notification.type] || "text-[#0060A9] bg-blue-50";
  ...
  return (
    ...
    >
      <div className="flex gap-3 p-3">
        <button onClick={handleNavigate} className="flex flex-1 items-start gap-3 text-left">
          <img
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="truncate text-sm font-bold text-gray-900">{notification.actorName}</p>
                <p className="mt-0.5 line-clamp-2 text-xs text-gray-600">{notification.content}</p>
              </div>
            ...
            {notification.caption && (
              <p className="mt-1 line-clamp-2 text-[11px] text-gray-400">{notification.caption}</p>
            )}
