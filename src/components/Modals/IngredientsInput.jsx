...
// ─── Common ingredient dictionary for autocomplete + fuzzy correction ─────────
 ...
 * Combines prefix matching + fuzzy matching on individual words.
 */
function getSuggestions(query, limit = 6) {
  ...
  const scored = INGREDIENT_DICT.map((ing) => {
    ...
    // Word-level prefix: any word in ingredient starts with query
    const words = lower.split(" ");
    if (words.some((w) => w.startsWith(q))) return { ing, score: 0.5 };

    // Fuzzy: best levenshtein between query and any word in the ingredient
...
/**
 * Resolve a submitted name against the dictionary.
 * - Exact match -> returns canonical ingredient name
 * - Close typo  -> returns corrected name (silent auto-fix)
 * - Nonsense    -> returns null (blocked)
 */
function resolveIngredient(name) {
  const q = name.toLowerCase().trim();

  // 1. Exact match
  const exact = INGREDIENT_DICT.find((i) => i.toLowerCase() === q);
  if (exact) return exact;

  // 2. Fuzzy — find closest within edit-distance threshold
  let bestIng = null;
  let bestDist = Infinity;

  for (const ing of INGREDIENT_DICT) {
    const lower = ing.toLowerCase();
    if (Math.abs(q.split(" ").length - lower.split(" ").length) > 1) continue;
    const dist = levenshtein(q, lower);
    if (dist < bestDist) { bestDist = dist; bestIng = ing; }
  }

  const threshold = q.length <= 5 ? 1 : q.length <= 9 ? 2 : 3;
  if (bestDist <= threshold) return bestIng;
...
/** Amount: whole numbers OR fractions like 1/2, 3/4, 1 1/2 */
...
const IngredientInput = ({
  ...
  const addIngredient = useCallback(() => {
    ...
    // Resolve against dictionary (exact match or silent typo fix or block)
    const resolved = resolveIngredient(raw);

    if (!resolved) {
      setNameErr("Not a recognized ingredient. Try the suggestions.");
      return;
    }
    ...
    setIngredients((prev) => [
      ...prev,
      {
        ...
        name:   resolved, // always the canonical dictionary form
        amount: amount.trim(),
        unit,
      },
    ]);
    ...
    setAmountErr(false);
    setShowSuggest(false);
  }, [name, amount, unit, setIngredients]);
          ...
          <div ref={containerRef} className="relative">
            <div className="flex gap-1.5 items-start">
              ...
              <div className="flex-1 flex flex-col gap-0.5 relative">
                ...
                />
                {nameErr && (
                  <span className="text-[9px] text-red-400 font-semibold leading-tight pl-1">
                    {nameErr}
                  </span>
                )}
