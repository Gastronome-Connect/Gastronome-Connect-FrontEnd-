...
export default function Features() {
  const [sectionRef, hasAnimated] = useScrollReveal();
  const prefersReducedMotion = useMemo(
    () => window.matchMedia("(prefers-reduced-motion: reduce)").matches,
    []
  );
  const getDelay = (index) => (prefersReducedMotion || !hasAnimated ? "0ms" : `${index * 150}ms`);
...
Gastronome Connect combines intelligent recipe generation with interactive social features, allowing users to discover, create, and share meals effortlessly in one unified platform.
          ...
          <div className="flex flex-col gap-5 sm:gap-8">
            {[
              ...
              <div
                ...
                className={`group border-2 border-[#0060A9] py-6 sm:py-10 px-6 sm:px-10 rounded-[20px] sm:rounded-[30px] bg-white flex gap-5 sm:gap-8 items-center shadow-lg cursor-pointer
        ...
        <div className="hidden lg:grid grid-cols-2 gap-6">
          <div className="flex flex-col gap-6">
            {[FirstImage, SecondImage].map((img, idx) => (
              <div
                ...
                className={`rounded-[50px] overflow-hidden shadow-xl cursor-pointer transition-all duration-500 ease-out
                  ${
                    prefersReducedMotion
                      ? "opacity-100 translate-y-0"
                      : hasAnimated
          ...
          <div
            ...
            className={`col-span-2 rounded-[50px] overflow-hidden mt-4 shadow-2xl cursor-pointer transition-all duration-500 ease-out
              ${
                prefersReducedMotion
                  ? "opacity-100 translate-y-0"
                  : hasAnimated
