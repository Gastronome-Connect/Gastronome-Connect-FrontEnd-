...
import { X, User, Calendar, Heart, Archive, ChevronRight } from "lucide-react";

const RecipeCard = ({ recipe, onDelete, variant }) => {
  const {
    title = "Binangkal",
    image = "",
    author = "Jomarrie",
  ...
  return (
    <motion.div
      ...
      onClick={() => navigate(`/recipe/${recipe?.id ?? 0}`)}
        ...
        <div className="absolute top-3 right-3 z-10">
          {variant === "favorite" ? (
            <button
              ...
              // DEFAULT: solid red fill = highlighted/active state
              // HOVER:   white background, red icon = dimmed/inactive state
            ...
            <button
              onClick={(e) => { e.stopPropagation(); onDelete?.(); }}
              // DEFAULT: solid orange fill = highlighted/active state
              // HOVER:   white background, orange icon = dimmed/inactive state
              // (inline style used for the non-Tailwind orange color)
...
/**
 * RecipeGrid — wrap your list with this for smooth delete transitions.
 * lg  = laptop  → 5 cols, tighter gaps
 * xl+ = desktop → 5 cols, normal gaps
 */
export const RecipeGrid = ({ recipes, onDelete, variant }) => {
  return (
